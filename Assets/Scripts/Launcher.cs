...
public class Launcher : MonoBehaviour
{
    ...
    public Dictionary<char, GameObject> tiles = new Dictionary<char, GameObject>();
    public string a;
    ...
    void Start()
    {
        ...
        string word5 = RandomWord(db.words5);
        List<char> datalist = new List<char>();
        datalist.AddRange(word5);
        int idx = -5;
        datalist.ForEach(c => Instantiate(tiles[c], new Vector3(idx++, 0, 0), tiles['A'].transform.rotation));
        //shuffle list
        datalist = datalist.OrderBy(i => Guid.NewGuid()).ToList();
        idx = -5;
        datalist.ForEach(c => Instantiate(tiles[c], new Vector3(idx++, -1, 0), tiles['A'].transform.rotation));
